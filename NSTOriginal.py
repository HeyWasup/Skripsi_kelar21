...
# ------------------------
'''
#Sebelumnya saya pakai yang ini, tapi karena error dan menurut chatgpt ini penyebanya salah satu entah contetn atau style nay itu mengandung 4
def load_image(image_path, max_size=512, device='cuda'):
    """
    Loads an image from disk, scales to max_size, and converts to a tensor.
    """
...
def load_image(image_path, max_size=512, device='cuda'):
    ...
    image = Image.open(image_path).convert("RGB")  # Pastikan gambar di-convert ke RGB
...
# ------------------------
# 2. Select Images Using Tkinter File Dialog
# ------------------------
...
# ------------------------
# 3. Load Fractal (Content) & Style Images
# ------------------------
# Gunakan GPU secara eksplisit:
device = torch.device("cuda")  # Pastikan GPU tersedia
fractal_img = load_image(content_path, max_size=512, device=device)
style_img   = load_image(style_path,   max_size=512, device=device)
# Initialize the generated image as a copy of the fractal content
generated_img = fractal_img.clone().requires_grad_(True)

# ------------------------
# 4. Load VGG19 for Feature Extraction
# ------------------------
...
with torch.no_grad():
    ...
    _, content_feats_ref = extractor(fractal_img)
...
content_weight = 1 #
style_weight = 1e6 #bukan perbandingan, ada perhitungan aliain

# ------------------------
# 7. Define an Optimizer & Loss Function
# ------------------------
